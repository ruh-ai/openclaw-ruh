...
app/
  ...
  (platform)/                     — Main authenticated area
    ...
    page.tsx                      — Inert root fallback; middleware redirects authenticated `/` requests to `/agents`
    tools/
    ...
    agents/
      ...
      create/
        ...
        _components/copilot/LifecycleStepRenderer.tsx — lifecycle stepper UI (embeds MeetYourEmployee for Reveal)
        _components/copilot/PendingQuestionsPanel.tsx — Renders `<ask_user>` markers as structured inputs above the chat input
        _components/copilot/ArtifactActionBar.tsx — Shared artifact controls for approve, revise, regenerate, compare, explain, and open-files actions
        _components/copilot/BuildReportPanel.tsx — Build readiness, blocker, setup, service-health, and verification panel
        _components/copilot/RequestChangesButton.tsx — "Ask architect to revise" button used on PRD/TRD/Plan artifacts
...
### v4 Harness Architecture (Think/Plan/Prototype/Build)

The agent creation lifecycle follows an 8-stage state machine after Reveal: `think` → `plan` → `prototype` → `build` → `review` → `test` → `ship` → `reflect`. The full lifecycle reference is in [[SPEC-agent-creation-lifecycle]]. Think, Plan, Prototype, and Build use the redesigned harness per [[SPEC-agent-creation-v3-build-pipeline]]:
...
**Plan** (`PLAN_SYSTEM_INSTRUCTION`): Structural architect. Reads PRD/TRD from workspace, produces structural decisions (skills, workflow, sub-agent ownership, data schema, API endpoints, dashboard pages, env vars) without generating file content (no inline `skillMd` or `soulContent`). Emits `<plan_skills>`, `<plan_workflow>`, `<plan_data_schema>`, `<plan_api_endpoints>`, `<plan_dashboard_pages>`, `<plan_dashboard_prototype>`, `<plan_sub_agents>`, `<plan_env_vars>`, `<plan_complete>` markers. When `dashboardPages` is not empty, the Plan must also include `dashboardPrototype`: the prototype approval gate that maps pages to operator workflows, required actions, review prompts, and acceptance criteria before Build can start. `StagePlan` renders this as a `Dashboard Prototype Gate` summary and an explicit `Sub-Agent Ownership` section when fleets are planned. Approving Plan unlocks Prototype; it does not start Build. Runtime env vars must carry `populationStrategy` (`user_required`, `ai_inferred`, or `static_default`) plus safe defaults where appropriate so ordinary booleans, limits, and paths do not become first-chat credential blockers. Writes `architecture.json` + `PLAN.md` to `.openclaw/plan/`. Falls back to JSON blob if markers fail.

**Prototype** (`LifecycleStepRenderer.tsx` + `dashboard-prototype.ts`): Frontend-rendered interactive dashboard prototype. It reads the approved `architecturePlan.dashboardPages`, `dashboardPrototype`, and `subAgents` from the Co-Pilot store and renders page navigation, mock dashboard components, workflows, required actions, pipeline tracking, generated artifacts, approval/revision controls, acceptance checks, revision prompts, and sub-agent ownership. The interaction is simulated in local UI state: operators can create a sample work item, run/advance the planned pipeline, simulate blockers, and approve or request revision on generated artifacts before Build starts. This stage does not generate files or start a preview server. `Approve Prototype & Start Build` is the only UI action that starts the server-side Build stream.

**Build** (`build-orchestrator.ts` + `build-harness.ts`): Build is now server-side. `CoPilotLayout.tsx` delegates to `POST /api/agents/:id/build` which returns a `stream_id`. The UI consumes SSE progress via `GET /api/agents/:id/build/stream/:stream_id`, processing `task_start`, `task_complete`, `task_failed`, `file_written`, `progress`, `status`, `setup_progress`, `build_report`, `build_complete`, and `error` events. Before specialists run, the backend normalizes the approved plan, copies the copilot workspace scaffold into the main agent workspace, and writes `.openclaw/plan/architecture.json` there so named specialist agents read the same approved artifact the operator reviewed. If an older workspace plan is missing `dashboardPrototype`, `POST /api/agents/:id/build` merges the approved prototype from `creation_session.coPilot.architecturePlan.dashboardPrototype` before scaffold generation. Long-running specialists emit their `task_start` and `status` events before the gateway call is awaited, keeping the Build UI live during database/backend/dashboard/verify work. Specialist execution is retry-safe: if the expected files for identity, skill chunks, database, backend, dashboard, or setup already exist in the main workspace, the server emits the same completion event and skips another gateway call instead of regenerating or timing out. Post-build verification uses `build-harness.ts` (v2): `generateVerificationPlan()` deterministically produces checks from the architecture plan (deps, TypeScript compile, dashboard build, database migration, backend service, dashboard routes, API endpoints, page quality), then the "verify" specialist runs inside the forge container with full tool access. After setup and verification, the backend writes an authoritative `.openclaw/build/build-report.json` to both the copilot workspace and the main workspace; its readiness (`blocked`, `test-ready`, or `ship-ready`) drives the builder store instead of relying only on optimistic UI state. Specialist prompts in `specialist-prompts.ts`. The deterministic scaffold writes JSX helpers as `.tsx`, sanitizes generated hook names from dashed API paths, renders approved dashboard prototype panels into matching dashboard pages — workflows as cards with arrow-flow step notation and required-action chips, pipeline as a horizontal stepper with numbered steps plus per-step approval-gate badges and produces-artifacts trails, and artifacts as review cards with type badge plus reviewAction buttons that fall back to a "no API endpoint planned" tooltip when unwired; `revisionPrompts` and `approvalChecklist` are review artifacts that gate Plan→Build in the builder UI and are intentionally NOT rendered into the runtime page, generates stateful sandbox API routes for prototype actions (create/reset work item, run pipeline, generate/approve/revise artifacts, resolve blockers, QA), uses bundler module resolution so the generated workspace compiles without import-extension rewrites, sources `.openclaw/.env` before local setup commands, and uses a passworded local Postgres `DATABASE_URL`. The previous client-side `runBuildPipeline()` path has been replaced.

**Workspace persistence**: Think/Plan outputs survive page refresh. `CoPilotLayout.tsx` rehydrates from workspace on mount via `readWorkspaceFile()`. File layout: `.openclaw/discovery/{research-brief,PRD,TRD}.md`, `.openclaw/plan/{architecture.json,PLAN.md,build-manifest.json}`, `.openclaw/build/{build-report,validation-report}.json`. Build manifest and build report writes are mirrored to both the copilot workspace and the main agent workspace so Review/Test always see the final manifest and readiness state after the workspace merge.

**Lifecycle gating**: `copilot-state.ts` enforces local hard gates per stage via `canAdvanceDevStage()`, while `PATCH /api/agents/:id/forge/stage` is the source of truth for committed stage advancement. Think requires `thinkStatus === "approved" | "done"` plus persisted PRD/TRD discovery documents before the UI can advance to Plan. `CoPilotLayout.tsx` writes the approved PRD/TRD into `.openclaw/discovery/` before confirming `forge_stage=plan`, so stage persistence cannot outrun the workspace artifacts Plan depends on. Plan requires `planStatus === "approved" | "done"` and, when the plan has dashboard pages, a non-empty approved `dashboardPrototype` before Prototype unlocks. Prototype requires a usable plan and the same dashboard prototype gate before Build can start. Build requires `buildStatus === "done"` before the UI asks the backend to advance. Review/Test require the persisted build report to be `test-ready` or `ship-ready`; Ship requires a passing backend eval result unless a development-only override is explicitly sent. `LifecycleStepRenderer.tsx` disables `Next` while the backend confirmation is pending and surfaces backend rejection text without moving local `devStage`.

**Event pipeline**: Markers in streamed text → `extractThinkMarkers()`/`extractPlanMarkers()`/`extractAskUserMarkers()` in `builder-agent.ts` → AG-UI custom events → `event-consumer-map.ts` consumers → `copilot-state.ts` Zustand store → UI re-renders. Key state fields: `thinkStep`, `researchFindings[]`, `planStep`, `planActivity[]`, `buildManifest`, `buildValidation`, `pendingQuestions[]` (checkpoint `<ask_user>` markers, cleared on user send).

**Collaborative checkpoints (2026-04-22)**: The architect now pauses at checkpoints in Think (C0 scope, C1 pre-PRD, C2 pre-TRD) and Plan (P0 skill boundaries), emitting `<ask_user>` markers that render as structured inputs in `PendingQuestionsPanel` above the chat input. The marker extractor/stripper accepts slash-containing option labels, source URLs, and placeholder-like values such as `<run_id>`, and each marker family tracks its own stream offset so overlap windows do not duplicate timeline events. Full behavior in [[SPEC-agent-creation-lifecycle]] § Collaborative Checkpoints.

**Pair-programmer queue (2026-05-11)**: The chat input send button is no longer gated by `isLoading` in builder mode. Submitting a message while the architect is mid-turn pushes it onto a local queue in `TabChat.tsx`; messages drain one-by-one via `useAgentChat.sendMessage` on the falling edge of `isLoading`. `QueuedMessagesChip.tsx` renders above the input bar showing the queue depth, with `Clear all` and per-message remove affordances. This is Phase 1 of the pair-programmer rollout — the iteration-loop side that lets queued messages actually steer mid-flight specialists is [[SPEC-pair-programmer-iteration-loop]] (proposed).

**Artifact-targeted revisions (2026-04-23, expanded 2026-04-26)**: `RequestChangesButton` still supports section-level PRD/TRD revision targeting, while `ArtifactActionBar` is now the shared top-level control surface for PRD/TRD, Plan, Build report, Review summary, and Test report artifacts. Those controls select the artifact target, set the appropriate chat mode (`ask` or `revise`), and keep approve/regenerate/compare/explain/open-files actions attached to the current lifecycle output instead of generic chat suggestions. Build also renders `BuildReportPanel`, which groups persisted `.openclaw/build/build-report.json` readiness into blockers, warnings, setup checks, service health, verification checks, and retry-failed-step affordances. Full behavior in [[SPEC-agent-creation-lifecycle]] § Artifact-Targeted Revisions.
...
That resume path now also fails closed when lifecycle markers outrun persisted artifacts. If a forging agent reopens with `forge_stage=plan` but no persisted PRD/TRD, the create flow returns to Think instead of showing a hollow Plan. If a forging agent reopens with `forge_stage=review` or later but still has no persisted `skillGraph`, the create flow no longer paints Think/Plan/Build as completed from that optimistic stage marker alone. The page also stops writing `review` or later back to `forge_stage` until build artifacts exist, which prevents a fast refresh from reopening on a green stepper backed by an empty build record. Review/Build transitions also treat `normalizePlan({})` fallback output as unusable and reload the on-disk `.openclaw/plan/architecture.json` before deriving stage state. [[LEARNING-2026-04-03-builder-forge-stage-truthfulness]] documents the root cause.
Route resume also treats `forge_stage` as backend truth for the visible lifecycle stage. Single-agent reads bypass browser cache and the backend agent read model must include both `forge_sandbox_id` and `forge_stage`; otherwise a resumed `/agents/create?agentId=...` page can fall back to stale Reveal state even when Postgres says the agent is already in Test or later. The reveal auto-trigger is suppressed until route restore completes and is skipped when the persisted stage is ahead of the current UI stage.

For Review-or-later resumes, backend skill graph truth also wins over empty cached skill selection state. A stale create-session cache may have `skillGraph: null` or `selectedSkillIds: []` from an earlier refresh; `buildResumedCoPilotSeed()` restores the persisted `skillGraph`, `workflow`, and selected skill ids so Ship readiness does not incorrectly block with "Select at least one resolved skill before deploy." Test-stage manual evaluation results remain blocked by default, but the UI now exposes explicit `Re-run Manual` and `Mark Manual Passed` actions so low-confidence results can be retried or manually adjudicated before `Approve Tests` / `Next` opens.
...
Plan failure is now surfaced visually. When `planStatus` is `"ready"` but `skillGenerationError` is set, `LifecycleStepRenderer.tsx` renders an error banner inside the Plan stage panel so operators see why `Approve Plan & Review Prototype` failed instead of getting a silent no-op.

The prototype-to-build handoff is now stricter as well. When the operator clicks `Approve Prototype & Start Build`, the build helper starts the server-side build from the approved `architecturePlan` already persisted in `.openclaw/plan/architecture.json`. That keeps Build aligned with the reviewed plan and prototype instead of re-inferring a detached dashboard or skill list.
...
`runtime-inputs.ts` is the shared contract layer for this package. It merges architect `required_env_vars`, skill-node `requires_env`, and saved operator-entered values into one runtime-input array, applies product-facing labels for the Google Ads proving case, and provides the completeness checks now used by review/deploy readiness. Plan enrichment ignores malformed env var rows and only projects entries with valid string keys, so a partially malformed architect plan cannot crash the create flow. Both builder shells now reuse the same `StepRuntimeInputs.tsx` editor: the Advanced Configure stepper and the default Co-Pilot Config-tab phase both present required-vs-optional badges, persist entered values through save and reopen, and keep deploy readiness honest when a required runtime value is still blank. The post-deploy setup page uses the same missing-input count to disable `Save & Continue` until required `user_required` values are filled, avoiding the previous quiet setup-to-chat redirect loop. The same fail-closed contract now gates both the page-level `Deploy Agent` CTA and the embedded Ship-stage activation path, and a failed `pushAgentConfig()` result now surfaces as a real completion error instead of falling through to a false-success Ship state.
...
- `use-sandbox-health.ts` polls `GET /api/sandboxes/:sandbox_id/status` so the agent list and deployed-agent header surface running vs stopped vs unreachable sandboxes from explicit runtime signals. The classifier accepts both older gateway status strings and the current reconciliation payload (`status: "live"`, `gateway_reachable`, `ok`, or `drift_state: "healthy"`) so a chat-capable sandbox is not mislabeled unreachable.
