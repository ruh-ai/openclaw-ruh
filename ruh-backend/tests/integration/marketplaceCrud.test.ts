...
beforeEach(async () => {
  ...
  const user = await userStore.createUser('publisher@ruh.ai', cachedHash, 'Publisher', 'developer');
  publisherId = user.id;
...
describe('Marketplace CRUD (integration)', () => {
  ...
  test('submit for review changes status', async () => {
    ...
    const updated = await marketplaceStore.updateListingStatus(listing.id, 'pending_review');
  ...
  test('install and uninstall', async () => {
    ...
    const installs = await marketplaceStore.listUserInstalls(publisherId);
    expect(installs.length).toBeGreaterThanOrEqual(1);

    const found = await marketplaceStore.getInstall(listing.id, publisherId);
    expect(found).not.toBeNull();
    expect(found!.version).toBe('1.0.0');

    await marketplaceStore.removeInstall(listing.id, publisherId);
    const after = await marketplaceStore.listUserInstalls(publisherId);
  ...
  test('search listings by text', async () => {
    ...
    const results = await marketplaceStore.listPublishedListings({ search: 'google' });
  ...
  test('search published listings by category', async () => {
    ...
    const results = await marketplaceStore.listPublishedListings({ category: 'analytics' });
    expect(results.total).toBeGreaterThanOrEqual(1);
    expect(results.items.every((l) => l.category === 'analytics')).toBe(true);

    const empty = await marketplaceStore.listPublishedListings({ category: 'nonexistent' });
