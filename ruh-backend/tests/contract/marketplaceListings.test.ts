...
const mockListPublished = mock(async () => ({ items: [makeFakeListing()], total: 1 }));
...
beforeEach(() => {
  mockListPublished.mockImplementation(async () => ({ items: [makeFakeListing()], total: 1 }));
...
describe('GET /api/marketplace/listings/:slug — response contract', () => {
  ...
  test('falls back to ID lookup when slug not found', async () => {
    ...
    mockGetById.mockImplementation(async () => makeFakeListing({ id: 'listing-by-id' }));
