 ...
 *
 * Only counts source files under src/ that were meaningfully tested — files
...
function flushFile() {
  ...
  const isSrc = currentFile.includes('/src/') || currentFile.startsWith('src/');
  const filePct = fileLF > 0 ? fileLH / fileLF : 0;

  if (isSrc && filePct >= MIN_FILE_COVERAGE) {
    linesHit += fileLH;
    linesFound += fileLF;
    fnHit += fileFNH;
    fnFound += fileFNF;
...
if (excluded.length > 0) {
  console.log(`[coverage] Excluded (below ${(MIN_FILE_COVERAGE * 100).toFixed(0)}% or non-src):`);
