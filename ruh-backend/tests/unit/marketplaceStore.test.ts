...
describe('slugify', () => {
  ...
  test('strips special characters', () => {
    expect(marketplaceStore.slugify('Agent @#$ Builder!')).toBe('agent-builder');
  });

  test('collapses multiple spaces and hyphens', () => {
    expect(marketplaceStore.slugify('  too   many   spaces  ')).toBe('too-many-spaces');
  });
...
describe('createListing', () => {
  test('inserts a listing with correct parameters', async () => {
    ...
    mockQuery.mockImplementation(async () => ({ rows: [fakeRow], rowCount: 1 }));
...
describe('listPublishedListings', () => {
  test('filters by published status', async () => {
    mockQuery.mockImplementation(async () => ({ rows: [{ count: '0' }], rowCount: 1 }));
...
describe('updateListingStatus', () => {
  test('sets published_at when status is published', async () => {
    ...
    await marketplaceStore.updateListingStatus('listing-1', 'published', 'admin-1', 'looks good');
...
describe('createInstall', () => {
  test('uses ON CONFLICT to upsert install', async () => {
    const fakeInstall = {
      id: 'test-uuid-1234-5678-9abc-def012345678',
      listing_id: 'listing-1',
      user_id: 'user-1',
    ...
    expect(sql).toContain('DO UPDATE SET version');
  });
});
...
describe('removeInstall', () => {
  test('returns true when install deleted', async () => {
    mockQuery.mockImplementation(async () => ({ rows: [], rowCount: 1 }));
    const result = await marketplaceStore.removeInstall('listing-1', 'user-1');
