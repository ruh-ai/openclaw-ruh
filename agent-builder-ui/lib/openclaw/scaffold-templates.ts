...
import type { ArchitecturePlan, DashboardPage, DashboardPrototypeAction, DashboardPrototypePage, DashboardPrototypeWorkflow } from "./types";
import { normalizePlan } from "./plan-formatter";
import { synthesizeFixtures, synthesizeTaskRuns } from "./preview-fixtures";
...
function actionEndpointScore(action: DashboardPrototypeAction, endpoint: NonNullable<ArchitecturePlan["apiEndpoints"]>[number]): number {
...
export function prototypeActionEndpointMap(plan: ArchitecturePlan): Record<string, { method: string; path: string }> {
...
// ── Tasks page template strings — kept in sync with ruh-backend mirror ────
...
export function TaskFeed({ tasks, selectedTaskId, onSelect }: {
  tasks: TaskSummary[]; selectedTaskId: string | null; onSelect: (id: string) => void;
}) {
...
const KIND_COLOR: Record<TimelineEventKind, string> = {
  input: tokens.info, step_started: tokens.primary, step_completed: tokens.success,
  tool_call: tokens.secondary, decision: tokens.primary, artifact_produced: tokens.primary,
  approval_requested: tokens.warning, approval_granted: tokens.success, approval_rejected: tokens.error,
  error: tokens.error, complete: tokens.success,
};
...
export function RunInspector({ run }: { run: TaskRunDetail | null }) {
  ...
  if (!run) return <div style={{ background: tokens.cardColor, border: \`1px solid \${tokens.borderDefault}\`, borderRadius: 12, padding: 40, textAlign: 'center', color: tokens.textTertiary }}>Select a task to inspect its run.</div>;
...
export function useTasks(): { tasks: TaskSummary[]; loading: boolean; error: string | null } {
  const [state, setState] = useState<{ tasks: TaskSummary[]; loading: boolean; error: string | null }>({ tasks: [], loading: true, error: null });
  useEffect(() => {
    let active = true;
    fetch('/api/tasks').then((r) => r.ok ? r.json() : Promise.reject(r.statusText)).then((json) => {
      ...
      if (items.length > 0) setState({ tasks: items, loading: false, error: null });
      else setState({ tasks: ((all.__tasks as { items?: TaskSummary[] } | undefined)?.items) ?? [], loading: false, error: null });
    }).catch(() => {
      if (!active) return;
      setState({ tasks: ((all.__tasks as { items?: TaskSummary[] } | undefined)?.items) ?? [], loading: false, error: null });
    });
    return () => { active = false; };
  }, []);
  return state;
...
export default function TasksPage() {
  ...
  useEffect(() => { if (!selectedTaskId && tasks.length > 0) setSelectedTaskId(tasks[0].id); }, [tasks, selectedTaskId]);
...
`;
  ...
  // ── fixtures.json — day-1 seed data (dataSources + reserved __tasks/__runs) ──
  const fixtureMap = synthesizeFixtures(plan) as Record<string, unknown>;
  const taskData = synthesizeTaskRuns(plan);
  fixtureMap.__tasks = { items: taskData.tasks };
  fixtureMap.__runs = taskData.runs;
  files.push({
    path: "dashboard/fixtures.json",
    content: JSON.stringify(fixtureMap, null, 2) + "\n",
  });

  // ── hooks/useApi.ts — falls back to fixtures.json on empty/failed response ──
  files.push({
