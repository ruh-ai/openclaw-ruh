...
function serializeListingRow(row: Record<string, unknown>): ListingRecord {
  return {
    ...
    screenshots: Array.isArray(row.screenshots) ? row.screenshots : JSON.parse(String(row.screenshots || '[]')),
  ...
  return withConn(async (client) => {
    ...
    const result = await client.query(
      `INSERT INTO marketplace_listings
         (id, agent_id, publisher_id, title, slug, summary, description, category, tags, icon_url, screenshots, version)
...
export async function getListingById(id: string): Promise<ListingRecord | null> {
  return withConn(async (client) => {
    const result = await client.query('SELECT * FROM marketplace_listings WHERE id = $1', [id]);
    return result.rows[0] ? serializeListingRow(result.rows[0]) : null;
  });
}

export async function getListingBySlug(slug: string): Promise<ListingRecord | null> {
  return withConn(async (client) => {
    const result = await client.query('SELECT * FROM marketplace_listings WHERE slug = $1', [slug]);
    return result.rows[0] ? serializeListingRow(result.rows[0]) : null;
  });
}
...
export async function listPublishedListings(filters?: {
  ...
  return withConn(async (client) => {
    ...
    if (filters?.search) {
      conditions.push(`(title ILIKE $${paramIdx} OR summary ILIKE $${paramIdx} OR description ILIKE $${paramIdx})`);
    ...
    const countResult = await client.query(`SELECT COUNT(*) FROM marketplace_listings ${where}`, params);
...
export async function listUserListings(publisherId: string): Promise<ListingRecord[]> {
  return withConn(async (client) => {
    const result = await client.query(
...
export async function updateListing(
  id: string,
  patch: Partial<Pick<ListingRecord, 'title' | 'summary' | 'description' | 'category' | 'tags' | 'iconUrl' | 'screenshots' | 'version'>>,
  ...
  return withConn(async (client) => {
    ...
    if (patch.tags !== undefined) { sets.push(`tags = $${idx++}`); params.push(JSON.stringify(patch.tags)); }
    if (patch.iconUrl !== undefined) { sets.push(`icon_url = $${idx++}`); params.push(patch.iconUrl); }
    if (patch.screenshots !== undefined) { sets.push(`screenshots = $${idx++}`); params.push(JSON.stringify(patch.screenshots)); }
  ...
  return withConn(async (client) => {
    ...
    const result = await client.query(
      `INSERT INTO marketplace_reviews (id, listing_id, user_id, rating, title, body)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [id, data.listingId, data.userId, data.rating, data.title ?? null, data.body ?? null],
    );
  ...
  return withConn(async (client) => {
    ...
    const result = await client.query(
      `INSERT INTO marketplace_installs (id, listing_id, user_id, version)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (listing_id, user_id) DO UPDATE SET version = $4, installed_at = NOW()
       RETURNING *`,
      [id, data.listingId, data.userId, data.version],
    );
    return serializeInstallRow(result.rows[0]);
  });
}

export async function removeInstall(listingId: string, userId: string): Promise<boolean> {
  return withConn(async (client) => {
    const result = await client.query(
      `DELETE FROM marketplace_installs WHERE listing_id = $1 AND user_id = $2`,
      [listingId, userId],
    );
...
export async function listUserInstalls(userId: string): Promise<InstallRecord[]> {
  return withConn(async (client) => {
    const result = await client.query(
      `SELECT * FROM marketplace_installs WHERE user_id = $1 ORDER BY installed_at DESC`,
      [userId],
    );
...
export async function getInstall(listingId: string, userId: string): Promise<InstallRecord | null> {
  return withConn(async (client) => {
    const result = await client.query(
      `SELECT * FROM marketplace_installs WHERE listing_id = $1 AND user_id = $2`,
      [listingId, userId],
    );
