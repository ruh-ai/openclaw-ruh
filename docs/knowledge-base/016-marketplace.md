...
New rows now stamp `owner_org_id` from the active developer org so teammates in the same developer org can see and manage the listing even though creation still requires the publishing user to own the referenced agent. The `repo_url` column (nullable) links V3 agent-as-code listings to their source repository; when present, the install flow triggers sandbox provisioning with repo clone + setup instead of instant snapshot-only install.
...
| POST | /api/marketplace/listings/:id/install | Customer org | Create or reuse the current user's installed runtime agent (V3 agents with `repo_url` also trigger sandbox provisioning) |
| DELETE | /api/marketplace/listings/:id/install | Customer org | Remove the current user's installed runtime agent (also deletes sandboxes) |
| POST | /api/marketplace/listings/auto-publish | Developer org | One-step create + publish for the Ship stage (idempotent — returns existing published listing if found) |
...
- `POST /listings/auto-publish` creates + publishes in one step for the Ship stage; idempotent — re-publishes draft/rejected listings and returns already-published ones as-is
- listing creation now accepts an optional `repoUrl`; if omitted, it falls back to the source agent's `repo_url`
- V3 install path: when the published snapshot has a `repoUrl`, install creates a sandbox provisioning stream (`streamId` + `provisioning: true` in response) instead of returning an instant install; V2 agents (no `repoUrl`) still install from snapshot immediately
