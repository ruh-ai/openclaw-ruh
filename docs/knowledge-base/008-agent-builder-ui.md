...
The default Co-Pilot shell now uses one builder workspace instead of a separate configure rail. Chat stays on the left, `TabChat.tsx` reuses its `Agent's Computer` panel on the right, and the `Config` tab hosts the live builder snapshot plus the active `purpose`/`skills`/`tools`/`runtime_inputs`/`triggers`/`review` step content from [[SPEC-copilot-config-workspace]]. Builder phase changes can pull focus back to `Config`, while runtime tool activity still auto-focuses `terminal`, `code`, or `browser` as appropriate.
...
When the page is editing an existing agent, `/agents/create?agentId=<id>` now also enters the same Co-Pilot workspace by default instead of dropping into the legacy advanced-chat shell. Route entry seeds the shared Co-Pilot store from the saved agent snapshot so purpose, selected skills, tool connections, runtime inputs, triggers, and accepted improvements are visible immediately inside the `purpose` → `skills` → `tools` → `runtime_inputs` → `triggers` → `review` flow.
...
- `TabChat.tsx` now exposes an `Agent's Computer` workspace panel with `terminal`, `code`, `files`, and `browser` tabs. The panel auto-switches tabs based on the active tool type (shell → terminal, file_write → code, browser_navigate → browser) with debounce and manual-override protection.
...
Page-route auth gating is now active per [[SPEC-agent-builder-auth-gate]]. In non-development environments, `middleware.ts` protects non-auth pages by redirecting requests without auth cookies to `/authenticate?redirect_url=...`, and `SessionInitializationWrapper.tsx` fails closed after hydration when user bootstrap proves the session is missing or unauthorized. The auth page stays public, but existing sessions are redirected back into the requested platform route instead of remaining on the login screen. In local development, both middleware and session bootstrap intentionally bypass auth so the builder can run against local backends without the production session loop.

The current auth/session implementation is still intentionally browser-readable: `authCookies.ts` sets both auth cookies with `httpOnly: false`, `SessionInitializationWrapper.tsx` still copies the access token into the persisted `useUserStore`, and client axios interceptors read/refresh those tokens directly. That model remains transitional and should not be extended by future auth work; the hardening follow-up is [[SPEC-agent-builder-session-token-hardening]]. This run only restores fail-closed page gating and cookie-clear parity under the current model.
